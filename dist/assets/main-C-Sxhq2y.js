...
削除すると順位表も更新されます。`))return;const{error:e}=await b.from("catches").delete().eq("id",t);if(e){console.error("❌ 削除エラー:",e),d("❌ 削除に失敗しました",!0);return}d("✅ 削除しました"),await D(),await C()};async function C(){console.log("🏆 ランキング計算開始"),console.log("📋 現在のCONFIG:",u),console.log("📊 リミット匹数:",u.limit_count),console.log("🎯 大会ルール:",u.rule_type);const t=u.hide_ranking===!0;if(console.log("🔒 順位非表示設定:",t,"(CONFIG.hide_ranking:",u.hide_ranking,")"),t&&k<2){console.log("🚫 順位は非表示に設定されています（管理者以外）"),document.getElementById("ranking-list").style.display="none",document.getElementById("ranking-hidden-message").style.display="block",document.getElementById("show-more-btn").style.display="none",document.getElementById("biggest-fish-list").innerHTML='<div class="empty-state">順位発表までお待ちください</div>',document.getElementById("smallest-fish-list").innerHTML='<div class="empty-state">順位発表までお待ちください</div>';return}document.getElementById("ranking-list").style.display="block",document.getElementById("ranking-hidden-message").style.display="none";const{data:e,error:n}=await b.from("catches").select("*").eq("tournament_id",f);if(n){console.error("❌ ランキング読み込みエラー:",n);return}const o=e||[];if(console.log("📊 釣果データ:",o.length,"件"),o.length===0){document.getElementById("ranking-list").innerHTML='<div class="empty-state">まだ釣果がありません</div>',document.getElementById("biggest-fish-list").innerHTML='<div class="empty-state">まだ釣果がありません</div>',document.getElementById("smallest-fish-list").innerHTML='<div class="empty-state">まだ釣果がありません</div>';return}const i={};w.forEach(c=>{i[c.zekken]=c});const l={};o.forEach(c=>{l[c.zekken]||(l[c.zekken]={zekken:c.zekken,lengths:[],weights:[],min_len:c.length,max_len:c.length,min_weight:c.weight||0,max_weight:c.weight||0}),l[c.zekken].lengths.push(c.length),l[c.zekken].weights.push(c.weight||0),l[c.zekken].min_len=Math.min(l[c.zekken].min_len,c.length),l[c.zekken].max_len=Math.max(l[c.zekken].max_len,c.length),l[c.zekken].min_weight=Math.min(l[c.zekken].min_weight,c.weight||0),l[c.zekken].max_weight=Math.max(l[c.zekken].max_weight,c.weight||0)});const s=Object.values(l).map(c=>{const p=[...c.lengths].sort((I,B)=>B-I),v=[...c.weights].sort((I,B)=>B-I),h=u.limit_count||999;console.log(`📊 選手${c.zekken}番の計算:`,{全釣果数:c.lengths.length,リミット匹数:h,全長寸:p,リミット長寸:p.slice(0,h)});const E=v.slice(0,h).reduce((I,B)=>I+B,0),L=p.slice(0,h).reduce((I,B)=>I+B,0);return{zekken:c.zekken,count:c.lengths.length,max_len:c.max_len,min_len:c.min_len,max_weight:c.max_weight,min_weight:c.min_weight,one_max_len:c.max_len,one_max_weight:c.max_weight,total_weight:c.weights.reduce((I,B)=>I+B,0),total_count:c.lengths.length,limit_weight:E,limit_total_len:L}}),r=u.rule_type||"max_len",a=u.sort1||null,m=u.sort2||null,y=u.sort3||null;s.sort((c,p)=>c[r]!==p[r]?p[r]-c[r]:a&&c[a]!==p[a]?p[a]-c[a]:m&&c[m]!==p[m]?p[m]-c[m]:y&&c[y]!==p[y]?p[y]-c[y]:0),V=s,console.log("✅ ランキング計算完了:",s.length,"人");const g=document.getElementById("show-biggest-fish")?.checked??!0;g?(document.querySelector(".prize-grid")?.style.setProperty("display","grid"),ye(s,i)):document.getElementById("biggest-fish-list").closest(".card").style.display="none";const x=document.getElementById("show-smallest-fish")?.checked??!0;x?(document.querySelector(".prize-grid")?.style.setProperty("display","grid"),fe(s,i)):document.getElementById("smallest-fish-list").closest(".card").style.display="none",!g&&!x&&document.querySelector(".prize-grid")?.style.setProperty("display","none"),ee(s,i)}function ye(t,e){const n=document.getElementById("biggest-fish-list").closest(".card");n.style.display="block";const o=[...t].sort((r,a)=>a.max_len===r.max_len?a.max_weight-r.max_weight:a.max_len-r.max_len),i=new Set,l=[];for(const r of o)if(!i.has(r.zekken)&&(l.push(r),i.add(r.zekken),l.length===3))break;const s=document.getElementById("biggest-fish-list");s.innerHTML=l.map((r,a)=>{const m=e[r.zekken]||{},y=m.name||"未登録",g=m.club||"";return`
    ...
    `).join("")}window.editPlayer=async function(t){const e=w.find(n=>n.zekken===t);if(!e){d("選手が見つかりません",!0);return}console.log("📝 編集前の選手情報:",e),he(e,async n=>{if(!n)return;console.log("📝 更新データ:",n),console.log("📝 更新条件:",{tournament_id:f,zekken:t});const{data:o,error:i}=await b.from("players").update({name:n.name,club:n.club,reading:n.reading}).eq("tournament_id",f).eq("zekken",t).select();if(i){console.error("❌ 選手編集エラー:",i),console.error("❌ エラー詳細:",JSON.stringify(i,null,2)),d(`❌ 編集に失敗しました: ${i.message||i.code||"不明なエラー"}`,!0);return}if(!o||o.length===0){console.error("❌ 更新対象が見つかりませんでした"),d("❌ 更新対象が見つかりませんでした",!0);return}console.log("✅ 更新後のデータ:",o),d("✅ 選手情報を更新しました"),await T(),await q(),console.log("✅ 再読み込み後のALL_PLAYERS:",w.find(l=>l.zekken===t))})};function he(t,e){const n=`
...
`,i.forEach((p,v)=>{const h=e.find(E=>E.zekken===p.zekken)||{};s+=`${v+1}位,${p.zekken}番,"${h.name||"未登録"}","${h.club||""}",${p.length},${p.weight||0}
`}),s+=`
...
`}));const a=u.name||"tournament",m=new Date().toISOString().split("T")[0],y=`${a}_完全版_${m}.csv`,g="\uFEFF",x=new Blob([g+s],{type:"text/csv;charset=utf-8;"}),c=document.createElement("a");c.href=URL.createObjectURL(x),c.download=y,c.click(),d("✅ CSVファイルをダウンロードしました")}catch(t){console.error("❌ エクスポートエラー:",t),d("❌ エクスポートに失敗しました",!0)}};document.addEventListener("DOMContentLoaded",function(){["zekken-number-input","length-input","weight-input"].forEach(e=>{const n=document.getElementById(e);n&&n.addEventListener("input",function(o){const i=o.target.value,l=X(i);i!==l&&(o.target.value=l)})})});window.exportPDF=async function(){try{if(d("📄 PDF生成中..."),typeof window.jspdf>"u"||typeof html2canvas>"u"){d("❌ PDFライブラリが読み込まれていません",!0);return}const{jsPDF:t}=window.jspdf,e=V||[],n=w||[];if(e.length===0){d("❌ まだ釣果データがありません",!0);return}const i={limit_total_len:"リミット合計長寸",limit_weight:"リミット合計重量",total_count:"総枚数",total_weight:"総重量"}[u.rule_type]||"リミット合計長寸",l=u.limit_count>0?`(リミット${u.limit_count}匹)`:"(無制限)",s=document.createElement("div");s.style.cssText=`
        ...
        `;const m=[],y=await ne(3);if(console.log("🏆 PDF大物賞データ:",y),y.length>0){let _=`
            ...
            `:console.log("⚠️ PDF特別賞データがありません");const{data:x,error:c}=await b.from("catches").select("*").eq("tournament_id",f).order("created_at",{ascending:!1});!c&&x&&x.length>0&&(s.innerHTML+=`

import{createClient as ce}from"https://cdn.jsdelivr.net/npm/@supabase/supabase-js@2/+esm";(function(){const t=document.createElement("link").relList;if(t&&t.supports&&t.supports("modulepreload"))return;for(const o of document.querySelectorAll('link[rel="modulepreload"]'))n(o);new MutationObserver(o=>{for(const r of o)if(r.type==="childList")for(const s of r.addedNodes)s.tagName==="LINK"&&s.rel==="modulepreload"&&n(s)}).observe(document,{childList:!0,subtree:!0});function i(o){const r={};return o.integrity&&(r.integrity=o.integrity),o.referrerPolicy&&(r.referrerPolicy=o.referrerPolicy),o.crossOrigin==="use-credentials"?r.credentials="include":o.crossOrigin==="anonymous"?r.credentials="omit":r.credentials="same-origin",r}function n(o){if(o.ep)return;o.ep=!0;const r=i(o);fetch(o.href,r)}})();const K="https://pkjvdtvomqzcnfhkqven.supabase.co",ue="eyJhbGciOiJIUzI1NiIsInR5cCI6IkpXVCJ9.eyJpc3MiOiJzdXBhYmFzZSIsInJlZiI6InBranZkdHZvbXF6Y25maGtxdmVuIiwicm9sZSI6ImFub24iLCJpYXQiOjE3NzA3NDU2MjYsImV4cCI6MjA4NjMyMTYyNn0.Wn-igVmMwRbmR9ph5uNC4_HdOdclEccqNQWimRP-C38",b=ce(K,ue);let k=0,u={},h=null,v=[],V=[],G=!0,R=null,Q=10,J=[];console.log("🎣 システム起動");document.addEventListener("DOMContentLoaded",async function(){const t=new URLSearchParams(window.location.search).get("id");t?await ge(t):te()});function te(){document.getElementById("top-page").style.display="flex",document.getElementById("tournament-page").style.display="none",ne()}window.enterTournament=function(){const e=document.getElementById("tournament-id-input").value.trim();if(!e){d("大会IDを入力してください",!0);return}window.location.href=`?id=${e}`};async function ne(){const{data:e,error:t}=await b.from("tournaments").select("id, name, created_at").order("created_at",{ascending:!1}).limit(10),i=document.getElementById("tournament-list");if(t){console.error("大会一覧読み込みエラー:",t),i.innerHTML='<div style="color: #e74c3c;">読み込みに失敗しました</div>';return}if(!e||e.length===0){i.innerHTML='<div style="opacity: 0.6;">まだ大会がありません</div>';return}i.innerHTML=e.map(n=>`
...
削除すると順位表も更新されます。`))return;const{error:t}=await b.from("catches").delete().eq("id",e);if(t){console.error("❌ 削除エラー:",t),d("❌ 削除に失敗しました",!0);return}d("✅ 削除しました"),await D(),await C()};async function C(){console.log("🏆 ランキング計算開始"),console.log("📋 現在のCONFIG:",u),console.log("📊 リミット匹数:",u.limit_count),console.log("🎯 大会ルール:",u.rule_type);const e=u.hide_ranking===!0;if(console.log("🔒 順位非表示設定:",e,"(CONFIG.hide_ranking:",u.hide_ranking,")"),e&&k<2){console.log("🚫 順位は非表示に設定されています（管理者以外）"),document.getElementById("ranking-list").style.display="none",document.getElementById("ranking-hidden-message").style.display="block",document.getElementById("show-more-btn").style.display="none",document.getElementById("biggest-fish-list").innerHTML='<div class="empty-state">順位発表までお待ちください</div>',document.getElementById("smallest-fish-list").innerHTML='<div class="empty-state">順位発表までお待ちください</div>';return}document.getElementById("ranking-list").style.display="block",document.getElementById("ranking-hidden-message").style.display="none";const{data:t,error:i}=await b.from("catches").select("*").eq("tournament_id",h);if(i){console.error("❌ ランキング読み込みエラー:",i);return}const n=t||[];if(console.log("📊 釣果データ:",n.length,"件"),n.length===0){document.getElementById("ranking-list").innerHTML='<div class="empty-state">まだ釣果がありません</div>',document.getElementById("biggest-fish-list").innerHTML='<div class="empty-state">まだ釣果がありません</div>',document.getElementById("smallest-fish-list").innerHTML='<div class="empty-state">まだ釣果がありません</div>';return}const o={};v.forEach(c=>{o[c.zekken]=c});const r={};n.forEach(c=>{r[c.zekken]||(r[c.zekken]={zekken:c.zekken,lengths:[],weights:[],min_len:c.length,max_len:c.length,min_weight:c.weight||0,max_weight:c.weight||0}),r[c.zekken].lengths.push(c.length),r[c.zekken].weights.push(c.weight||0),r[c.zekken].min_len=Math.min(r[c.zekken].min_len,c.length),r[c.zekken].max_len=Math.max(r[c.zekken].max_len,c.length),r[c.zekken].min_weight=Math.min(r[c.zekken].min_weight,c.weight||0),r[c.zekken].max_weight=Math.max(r[c.zekken].max_weight,c.weight||0)});const s=Object.values(r).map(c=>{const g=[...c.lengths].sort(($,_)=>_-$),x=[...c.weights].sort(($,_)=>_-$),w=u.limit_count||999;console.log(`📊 選手${c.zekken}番の計算:`,{全釣果数:c.lengths.length,リミット匹数:w,全長寸:g,リミット長寸:g.slice(0,w)});const z=x.slice(0,w).reduce(($,_)=>$+_,0),L=g.slice(0,w).reduce(($,_)=>$+_,0);return{zekken:c.zekken,count:c.lengths.length,max_len:c.max_len,min_len:c.min_len,max_weight:c.max_weight,min_weight:c.min_weight,one_max_len:c.max_len,one_max_weight:c.max_weight,total_weight:c.weights.reduce(($,_)=>$+_,0),total_count:c.lengths.length,limit_weight:z,limit_total_len:L}}),l=u.rule_type||"max_len",a=u.sort1||null,m=u.sort2||null,y=u.sort3||null;s.sort((c,g)=>c[l]!==g[l]?g[l]-c[l]:a&&c[a]!==g[a]?g[a]-c[a]:m&&c[m]!==g[m]?g[m]-c[m]:y&&c[y]!==g[y]?g[y]-c[y]:0),J=s,console.log("✅ ランキング計算完了:",s.length,"人");const p=document.getElementById("show-biggest-fish")?.checked??!0;p?(document.querySelector(".prize-grid")?.style.setProperty("display","grid"),he(s,o)):document.getElementById("biggest-fish-list").closest(".card").style.display="none";const f=document.getElementById("show-smallest-fish")?.checked??!0;f?(document.querySelector(".prize-grid")?.style.setProperty("display","grid"),be(s,o)):document.getElementById("smallest-fish-list").closest(".card").style.display="none",!p&&!f&&document.querySelector(".prize-grid")?.style.setProperty("display","none"),re(s,o)}function he(e,t){const i=document.getElementById("biggest-fish-list").closest(".card");i.style.display="block";const n=[...e].sort((l,a)=>a.max_len===l.max_len?a.max_weight-l.max_weight:a.max_len-l.max_len),o=new Set,r=[];for(const l of n)if(!o.has(l.zekken)&&(r.push(l),o.add(l.zekken),r.length===3))break;const s=document.getElementById("biggest-fish-list");s.innerHTML=r.map((l,a)=>{const m=t[l.zekken]||{},y=m.name||"未登録",p=m.club||"";let f,c,g,x;return a===0?(f="linear-gradient(135deg, #FFD700, #FFA500)",c="#FFD700",g="#000",x="🥇"):a===1?(f="linear-gradient(135deg, #C0C0C0, #A8A8A8)",c="#C0C0C0",g="#000",x="🥈"):(f="linear-gradient(135deg, #CD7F32, #B87333)",c="#CD7F32",g="#FFF",x="🥉"),`
            ...
            ">
                <div class="ranking-header">
                    <div style="font-size: 20px; font-weight: bold; color: ${g};">
                        ${x} ${a+1}位
                    </div>
                    <div>
                        <div style="font-size: 16px; font-weight: bold; color: ${g};">${l.zekken}番: ${y}</div>
                        ${p?`<div style="font-size: 12px; opacity: 0.8; color: ${g};">${p}</div>`:""}
                    </div>
                </div>
                <div class="ranking-stats">
                    <div class="stat">
                        <div class="stat-label" style="font-size: 12px; color: ${g}; opacity: 0.9;">最大長寸</div>
                        <div class="stat-value" style="color: ${g}; font-size: 20px; font-weight: bold;">${l.max_len.toFixed(1)}cm</div>
                    </div>
                </div>
            </div>
        `}).join("")}function be(e,t){const i=document.getElementById("smallest-fish-list").closest(".card");i.style.display="block";const n=[...e].sort((l,a)=>l.min_len===a.min_len?l.min_weight-a.min_weight:l.min_len-a.min_len),o=new Set,r=[];for(const l of n)if(!o.has(l.zekken)&&(r.push(l),o.add(l.zekken),r.length===3))break;const s=document.getElementById("smallest-fish-list");s.innerHTML=r.map((l,a)=>{const m=t[l.zekken]||{},y=m.name||"未登録",p=m.club||"";let f,c,g,x;return a===0?(f="linear-gradient(135deg, #FFD700, #FFA500)",c="#FFD700",g="#000",x="🥇"):a===1?(f="linear-gradient(135deg, #C0C0C0, #A8A8A8)",c="#C0C0C0",g="#000",x="🥈"):(f="linear-gradient(135deg, #CD7F32, #B87333)",c="#CD7F32",g="#FFF",x="🥉"),`
            ...
            ">
                ...
                <div class="ranking-stats">
                    <div class="stat">
                        <div class="stat-label" style="font-size: 12px; color: ${g}; opacity: 0.9;">最小長寸</div>
                        <div class="stat-value" style="color: ${g}; font-size: 20px; font-weight: bold;">${l.min_len.toFixed(1)}cm</div>
                    </div>
                </div>
            </div>
        `}).join("")}function ie(){const t=getComputedStyle(document.documentElement).getPropertyValue("--primary-color").trim().replace("#",""),i=parseInt(t.substr(0,2),16),n=parseInt(t.substr(2,2),16),o=parseInt(t.substr(4,2),16);return(i*299+n*587+o*114)/1e3>180}function re(e,t){const i=u.rule_type||"max_len",n=u.sort1||null,o=u.sort2||null,r=u.limit_count||0,s=ie(),l=s?"#1a1a1a":"white",a=s?"0.7":"0.9",m=Math.min(Q,e.length),y=e.slice(0,m),p=document.getElementById("ranking-list");p.innerHTML=y.map((c,g)=>{const x=g<3,w=t[c.zekken]||{},z=w.name||"未登録",L=w.club||"";let $="",_="transparent",P="2px",M="0 4px 8px rgba(0,0,0,0.2)";g===0?($="🏆",_="#FFD700",P="4px",M="0 6px 16px rgba(255, 215, 0, 0.4)"):g===1?($="🥈",_="#C0C0C0",P="4px",M="0 6px 16px rgba(192, 192, 192, 0.4)"):g===2&&($="🥉",_="#CD7F32",P="4px",M="0 6px 16px rgba(205, 127, 50, 0.4)");let N=Y[i];(i==="limit_total_len"||i==="limit_weight")&&r>0&&(N+=` (${r}匹)`);const I=U(i,c[i]),E=n?U(n,c[n]):null,B=o?U(o,c[o]):null;return`
            <div class="ranking-item ${x?"top3":""}" onclick="showPlayerDetail(${c.zekken})" style="cursor: pointer; transition: all 0.3s ease; background: linear-gradient(135deg, var(--primary-color), var(--secondary-color)); border-radius: 12px; padding: 15px; margin-bottom: 12px; box-shadow: ${M}; border: ${P} solid ${_};">
                <div style="display: flex; align-items: flex-start; gap: 12px; margin-bottom: 15px;">
                    <div style="display: flex; align-items: center; gap: 8px; flex-shrink: 0;">
                        ...
                        <span style="font-size: 32px; font-weight: bold; color: ${l}; white-space: nowrap;">${g+1}位</span>
                    </div>
        ...
        `}).join("");const f=document.getElementById("show-more-btn");e.length>Q?f.style.display="block":f.style.display="none"}window.showMoreRankings=function(){Q+=10;const e={};v.forEach(t=>{e[t.zekken]=t}),re(J,e),d("10件追加表示しました")};function U(e,t){return e.includes("len")?`${t.toFixed(1)}cm`:e.includes("weight")?`${Math.round(t)}g`:e==="total_count"?`${t}枚`:t}async function O(){const{data:e,error:t}=await b.from("players").select("*").eq("tournament_id",h).order("zekken");if(t){console.error("選手リスト読み込みエラー:",t);return}const i=e||[],n=document.getElementById("player-list");if(i.length===0){n.innerHTML='<div class="empty-state">選手が登録されていません</div>';return}n.innerHTML=i.map(o=>`
...
`,e.forEach((g,x)=>{const w=t.find(z=>z.zekken===g.zekken)||{};s+=`${x+1},${g.zekken},"${w.name||"未登録"}","${w.club||""}",${g.limit_total_len||0},${g.one_max_len||0},${g.one_max_weight||0},${g.total_count||0},${g.total_weight||0}
`}),s+=`
`,s+=`【特別賞】
        ...
        `;const l=u.name||"釣り大会",a=new Date().toLocaleDateString("ja-JP");s.innerHTML=`
            ...
            <div style="border: 1px solid #ddd; border-radius: 8px; overflow: hidden;">
                <table style="width: 100%; border-collapse: collapse;">
                    ...
                    <tbody>
                        ${t.map((I,E)=>{const B=i.find(F=>F.zekken===I.zekken)||{},T=U(u.rule_type,I[u.rule_type]);return`

import{createClient as re}from"https://cdn.jsdelivr.net/npm/@supabase/supabase-js@2/+esm";(function(){const e=document.createElement("link").relList;if(e&&e.supports&&e.supports("modulepreload"))return;for(const n of document.querySelectorAll('link[rel="modulepreload"]'))i(n);new MutationObserver(n=>{for(const l of n)if(l.type==="childList")for(const s of l.addedNodes)s.tagName==="LINK"&&s.rel==="modulepreload"&&i(s)}).observe(document,{childList:!0,subtree:!0});function o(n){const l={};return n.integrity&&(l.integrity=n.integrity),n.referrerPolicy&&(l.referrerPolicy=n.referrerPolicy),n.crossOrigin==="use-credentials"?l.credentials="include":n.crossOrigin==="anonymous"?l.credentials="omit":l.credentials="same-origin",l}function i(n){if(n.ep)return;n.ep=!0;const l=o(n);fetch(n.href,l)}})();const se="https://pkjvdtvomqzcnfhkqven.supabase.co",ae="eyJhbGciOiJIUzI1NiIsInR5cCI6IkpXVCJ9.eyJpc3MiOiJzdXBhYmFzZSIsInJlZiI6InBranZkdHZvbXF6Y25maGtxdmVuIiwicm9sZSI6ImFub24iLCJpYXQiOjE3NzA3NDU2MjYsImV4cCI6MjA4NjMyMTYyNn0.Wn-igVmMwRbmR9ph5uNC4_HdOdclEccqNQWimRP-C38",b=re(se,ae);let k=0,u={},h=null,w=[],N=[],V=!0,F=null,J=10,A=[];console.log("🎣 システム起動");document.addEventListener("DOMContentLoaded",async function(){const e=new URLSearchParams(window.location.search).get("id");e?await de(e):Z()});function Z(){document.getElementById("top-page").style.display="flex",document.getElementById("tournament-page").style.display="none",G()}window.enterTournament=function(){const t=document.getElementById("tournament-id-input").value.trim();if(!t){d("大会IDを入力してください",!0);return}window.location.href=`?id=${t}`};async function G(){const{data:t,error:e}=await b.from("tournaments").select("id, name, created_at").order("created_at",{ascending:!1}).limit(10),o=document.getElementById("tournament-list");if(e){console.error("大会一覧読み込みエラー:",e),o.innerHTML='<div style="color: #e74c3c;">読み込みに失敗しました</div>';return}if(!t||t.length===0){o.innerHTML='<div style="opacity: 0.6;">まだ大会がありません</div>';return}o.innerHTML=t.map(i=>`
    ...
    `).join("")}window.createTournament=async function(){const t=document.getElementById("new-tournament-id").value.trim(),e=document.getElementById("new-tournament-name").value.trim(),o=document.getElementById("new-tournament-admin-password").value.trim(),i=document.getElementById("new-tournament-staff-password").value.trim();if(!t||!e||!o){d("大会ID、大会名、管理者パスワードは必須です",!0);return}if(!/^[a-zA-Z0-9]+$/.test(t)){d("大会IDは半角英数字のみで入力してください",!0);return}console.log("🆕 大会作成:",{id:t,name:e});const{data:n,error:l}=await b.from("tournaments").insert({id:t,name:e,password:o,staff_password:i||null,rule_type:"limit_total_len",limit_count:0,sort1:"one_max_len",sort2:"one_max_weight",sort3:null}).select();if(l){console.error("大会作成エラー:",l),l.code==="23505"?d("この大会IDは既に使用されています",!0):d("大会の作成に失敗しました",!0);return}d("✅ 大会を作成しました！"),document.getElementById("new-tournament-id").value="",document.getElementById("new-tournament-name").value="",document.getElementById("new-tournament-admin-password").value="",document.getElementById("new-tournament-staff-password").value="",await G(),setTimeout(()=>{window.location.href=`?id=${t}`},1500)};async function de(t){h=t,console.log("📂 大会ID:",h);const{data:e,error:o}=await b.from("tournaments").select("*").eq("id",h).single();if(o||!e){console.error("大会取得エラー:",o),alert("大会が見つかりません"),Z();return}u=e,console.log("✅ 大会情報取得:",u),console.log("📋 大会ルール:",u.rule_type),console.log("📊 リミット匹数:",u.limit_count),console.log("🎯 優先順位1:",u.sort1),console.log("🎯 優先順位2:",u.sort2),console.log("🎯 優先順位3:",u.sort3),document.getElementById("tournament-name").textContent=u.name;const i=u.limit_count>0?`リミット${u.limit_count}匹`:"総力戦";document.getElementById("tournament-info").textContent=i,document.getElementById("top-page").style.display="none",document.getElementById("tournament-page").style.display="block",await C(),await L(),ve(),k===2&&(document.getElementById("tournament-management-card").style.display="block",W()),X(),ce()}function ce(){F&&F.unsubscribe(),F=b.channel("tournament-updates").on("postgres_changes",{event:"*",schema:"public",table:"catches",filter:`tournament_id=eq.${h}`},()=>{V&&(console.log("⚡ リアルタイム更新"),L(),k>0&&M())}).subscribe(),console.log("📡 リアルタイム購読開始")}window.toggleRealtimeUpdate=function(){V=document.getElementById("realtime-toggle").checked;const t=document.getElementById("manual-refresh-btn");V?(t.style.display="none",d("✅ リアルタイム更新: ON"),console.log("📡 リアルタイム更新: ON")):(t.style.display="inline-block",d("⏸️ リアルタイム更新: OFF（手動更新モード）"),console.log("⏸️ リアルタイム更新: OFF"))};window.manualRefreshRanking=async function(){d("🔄 更新中..."),await L(),k>0&&await M(),d("✅ 更新しました")};window.switchTab=function(t){document.querySelectorAll(".tab").forEach((o,i)=>{o.classList.remove("active"),(t==="ranking"&&i===0||t==="input"&&i===1||t==="settings"&&i===2)&&o.classList.add("active")}),document.querySelectorAll(".view").forEach(o=>{o.classList.remove("active")}),t==="ranking"?(document.getElementById("ranking-view").classList.add("active"),L()):t==="input"?(document.getElementById("input-view").classList.add("active"),k>0?(document.getElementById("login-box").style.display="none",document.getElementById("input-form").style.display="block",C(),M()):(document.getElementById("login-box").style.display="block",document.getElementById("input-form").style.display="none")):t==="settings"&&(document.getElementById("settings-view").classList.add("active"),k===2&&(document.getElementById("rule-settings-card").style.display="block",xe()),k>0&&C().then(()=>R()))};window.login=function(){const t=document.getElementById("password-input").value;if(t===u.password)k=2,d("✅ 管理者としてログイン"),Y("管理者");else if(t===u.staff_password)k=1,d("✅ 運営スタッフとしてログイン"),Y("運営スタッフ");else{d("パスワードが違います",!0);return}console.log("🔐 ログイン成功 AUTH_LEVEL:",k),document.getElementById("login-box").style.display="none",document.getElementById("input-form").style.display="block",k===2&&(document.getElementById("tournament-management-card").style.display="block",W()),C(),M()};window.logout=function(){ke("ログアウトしますか？",()=>{k=0,F&&(F.unsubscribe(),F=null),d("ログアウトしました"),console.log("🔓 ログアウト"),window.location.href="/"})};function Y(t){const e=document.getElementById("login-status"),o=document.getElementById("login-status-text");o.textContent=`${t}としてログイン中`,e.style.display="block"}async function C(){console.log("👥 選手データ読み込み開始");const{data:t,error:e}=await b.from("players").select("*").eq("tournament_id",h).order("zekken");if(e){console.error("❌ 選手読み込みエラー:",e);return}w=t||[],console.log("✅ 選手データ読み込み完了:",w.length,"人"),w.length>0&&console.log("📋 選手サンプル:",w[0]);const o=document.getElementById("player-select");o.innerHTML='<option value="">選手を選択してください</option>',w.forEach(i=>{const n=document.createElement("option");n.value=i.zekken,n.textContent=`${i.zekken}番: ${i.name}${i.club?` (${i.club})`:""}`,o.appendChild(n)})}function Q(t){return t.replace(/[Ａ-Ｚａ-ｚ０-９]/g,function(e){return String.fromCharCode(e.charCodeAt(0)-65248)})}function ue(t){return t.replace(/[\u30A1-\u30F6]/g,function(e){const o=e.charCodeAt(0)-96;return String.fromCharCode(o)})}function ge(t){return t.replace(/[\u3041-\u3096]/g,function(e){const o=e.charCodeAt(0)+96;return String.fromCharCode(o)})}function O(t){if(!t)return{original:"",hiragana:"",katakana:"",halfWidth:""};const e=ue(t),o=ge(t),i=Q(t);return{original:t,hiragana:e,katakana:o,halfWidth:i}}window.searchPlayer=function(){const t=document.getElementById("player-search"),e=document.getElementById("clear-search-btn"),o=document.getElementById("search-result-count"),i=document.getElementById("player-select"),n=t.value.trim();if(console.log("🔍 検索クエリ:",n),console.log("🔍 選手データ数:",w.length),w.length>0&&(console.log("📋 選手データサンプル（最初の3人）:"),w.slice(0,3).forEach(r=>{console.log(`  - ${r.zekken}番: ${r.name} (${r.club||"所属なし"})`)})),e.style.display=n?"block":"none",!n){i.innerHTML='<option value="">選手を選択してください</option>',w.forEach(r=>{const a=document.createElement("option");a.value=r.zekken,a.textContent=`${r.zekken}番: ${r.name}${r.club?` (${r.club})`:""}`,i.appendChild(a)}),o.textContent="";return}const l=O(n);console.log("🔧 正規化された検索クエリ:",{元:l.original,ひらがな:l.hiragana,カタカナ:l.katakana,半角:l.halfWidth});const s=w.filter(r=>{if(r.zekken.toString()===n||r.zekken.toString()===l.halfWidth)return console.log("✅ ゼッケン一致:",r.zekken),!0;if(r.reading){const a=O(r.reading);if(r.reading.includes(n))return console.log("✅ 読み仮名一致（完全）:",r.reading,"検索:",n),!0;if(a.hiragana.includes(l.hiragana)&&l.hiragana!=="")return console.log("✅ 読み仮名一致（ひらがな）:",r.reading,"検索:",n),!0;if(a.katakana.includes(l.katakana)&&l.katakana!=="")return console.log("✅ 読み仮名一致（カタカナ）:",r.reading,"検索:",n),!0}if(r.name){const a=O(r.name);if(r.name.includes(n))return console.log("✅ 名前一致（完全）:",r.name,"検索:",n),!0;if(a.hiragana.includes(l.hiragana)&&l.hiragana!=="")return console.log("✅ 名前一致（ひらがな）:",r.name,"検索:",n),!0;if(a.katakana.includes(l.katakana)&&l.katakana!=="")return console.log("✅ 名前一致（カタカナ）:",r.name,"検索:",n),!0;if(a.halfWidth.includes(l.halfWidth)&&l.halfWidth!=="")return console.log("✅ 名前一致（半角）:",r.name,"検索:",n),!0;const g=r.name.toLowerCase(),m=n.toLowerCase();if(g.includes(m))return console.log("✅ 名前一致（英語）:",r.name,"検索:",n),!0}if(r.club){const a=O(r.club);if(r.club.includes(n))return console.log("✅ 所属一致（完全）:",r.club,"検索:",n),!0;if(a.hiragana.includes(l.hiragana)&&l.hiragana!=="")return console.log("✅ 所属一致（ひらがな）:",r.club,"検索:",n),!0;if(a.katakana.includes(l.katakana)&&l.katakana!=="")return console.log("✅ 所属一致（カタカナ）:",r.club,"検索:",n),!0;if(a.halfWidth.includes(l.halfWidth)&&l.halfWidth!=="")return console.log("✅ 所属一致（半角）:",r.club,"検索:",n),!0;const g=r.club.toLowerCase(),m=n.toLowerCase();if(g.includes(m))return console.log("✅ 所属一致（英語）:",r.club,"検索:",n),!0}return!1});console.log("🔍 検索結果:",s.length,"件"),i.innerHTML='<option value="">選手を選択してください</option>',s.length===0?(o.textContent="該当する選手が見つかりません",o.style.color="#ff6b6b"):(s.forEach(r=>{const a=document.createElement("option");a.value=r.zekken,a.textContent=`${r.zekken}番: ${r.name}${r.club?` (${r.club})`:""}`,i.appendChild(a)}),o.textContent=`${s.length}件の選手が見つかりました`,o.style.color="#51cf66",s.length===1&&(i.value=s[0].zekken))};window.clearSearch=function(){const t=document.getElementById("player-search"),e=document.getElementById("clear-search-btn"),o=document.getElementById("search-result-count"),i=document.getElementById("player-select");t.value="",e.style.display="none",o.textContent="",i.innerHTML='<option value="">選手を選択してください</option>',w.forEach(n=>{const l=document.createElement("option");l.value=n.zekken,l.textContent=`${n.zekken}番: ${n.name}${n.club?` (${n.club})`:""}`,i.appendChild(l)})};window.switchInputMode=function(t){const e=document.getElementById("zekken-input-mode"),o=document.getElementById("search-input-mode"),i=document.getElementById("tab-zekken"),n=document.getElementById("tab-search");t==="zekken"?(e.style.display="block",o.style.display="none",i.style.background="linear-gradient(135deg, #667eea 0%, #764ba2 100%)",i.style.color="white",i.style.border="none",i.style.boxShadow="0 4px 15px rgba(102, 126, 234, 0.4)",n.style.background="rgba(255, 255, 255, 0.1)",n.style.color="rgba(255, 255, 255, 0.6)",n.style.border="2px solid rgba(255, 255, 255, 0.2)",n.style.boxShadow="none",setTimeout(()=>{document.getElementById("zekken-input").focus()},100)):(e.style.display="none",o.style.display="block",n.style.background="linear-gradient(135deg, #667eea 0%, #764ba2 100%)",n.style.color="white",n.style.border="none",n.style.boxShadow="0 4px 15px rgba(102, 126, 234, 0.4)",i.style.background="rgba(255, 255, 255, 0.1)",i.style.color="rgba(255, 255, 255, 0.6)",i.style.border="2px solid rgba(255, 255, 255, 0.2)",i.style.boxShadow="none",setTimeout(()=>{document.getElementById("player-search").focus()},100))};window.onZekkenInput=function(){const t=document.getElementById("zekken-input"),e=document.getElementById("player-info-display"),o=document.getElementById("player-name-display"),i=document.getElementById("player-club-display"),n=document.getElementById("player-error-display"),l=parseInt(t.value);if(!l||isNaN(l)){e.style.display="none",n.style.display="none";return}const s=w.find(r=>r.zekken===l);s?(e.style.display="block",n.style.display="none",o.textContent=`${s.zekken}番: ${s.name}`,i.textContent=s.club?`所属: ${s.club}`:"所属なし",console.log("✅ 選手が見つかりました:",s)):(e.style.display="none",n.style.display="block",console.log("❌ 選手が見つかりません:",l))};window.registerCatch=async function(){if(k===0){d("ログインが必要です",!0);return}const t=document.getElementById("zekken-input-mode").style.display!=="none";let e;t?e=parseInt(document.getElementById("zekken-input").value):e=parseInt(document.getElementById("player-select").value);const o=parseFloat(document.getElementById("length-input").value),i=parseFloat(document.getElementById("weight-input").value)||0;if(console.log("📝 登録データ:",{zekken:e,length:o,weight:i,mode:t?"ゼッケン":"検索"}),!e){d("選手を選択してください",!0);return}if(!o||o<=0){d("長寸を入力してください",!0);return}const n=w.find(r=>r.zekken==e);if(!n){d("選手が見つかりません",!0);return}const l=n.name,{error:s}=await b.from("catches").insert({tournament_id:h,zekken:e,length:o,weight:i});if(s){console.error("❌ 登録エラー:",s),d("登録に失敗しました",!0);return}console.log("✅ 登録成功"),d(`✅ ${l}: ${o}cm ${i>0?i+"g":""} を登録しました！`),t?(document.getElementById("zekken-input").value="",document.getElementById("player-info-display").style.display="none",document.getElementById("player-error-display").style.display="none",document.getElementById("zekken-input").focus()):document.getElementById("player-select").value="",document.getElementById("length-input").value="",document.getElementById("weight-input").value="",await M(),await L()};async function M(){console.log("📋 履歴読み込み開始"),console.log("👥 ALL_PLAYERS:",w);const t={};w.forEach(n=>{t[n.zekken]=n.name}),console.log("🗺️ playerMap:",t);const{data:e,error:o}=await b.from("catches").select("*").eq("tournament_id",h).order("created_at",{ascending:!1}).limit(50);if(o){console.error("❌ 履歴読み込みエラー:",o);return}N=e||[],console.log("✅ 履歴読み込み完了:",N.length,"件");const i=document.getElementById("history-list");if(N.length===0){i.innerHTML='<div class="empty-state">まだ履歴がありません</div>';return}i.innerHTML=N.map(n=>{const l=t[n.zekken]||"未登録",s=new Date(n.created_at).toLocaleString("ja-JP",{month:"2-digit",day:"2-digit",hour:"2-digit",minute:"2-digit"});return`
        ...
        `}).join("")}window.editCatch=async function(t,e,o,i){if(k!==2){d("管理者権限が必要です",!0);return}const n=w.find(s=>s.zekken===e),l=n?n.name:`${e}番`;me(t,e,l,o,i)};function me(t,e,o,i,n){const l=`
    ...
    `;document.body.insertAdjacentHTML("beforeend",l);const s=document.getElementById("edit-catch-dialog"),r=document.getElementById("edit-length-input"),a=document.getElementById("edit-weight-input"),g=document.getElementById("edit-catch-cancel-btn"),m=document.getElementById("edit-catch-save-btn");g.onclick=()=>{s.remove()},m.onclick=async()=>{const p=parseFloat(r.value),c=parseFloat(a.value)||0;if(!p||p<=0){d("長寸を入力してください",!0);return}s.remove();const{error:f}=await b.from("catches").update({length:p,weight:c}).eq("id",t);if(f){console.error("❌ 更新エラー:",f),d("❌ 更新に失敗しました",!0);return}d(`✅ ${o}の釣果を更新しました`),await M(),await L()},r.addEventListener("keypress",p=>{p.key==="Enter"&&m.click()}),a.addEventListener("keypress",p=>{p.key==="Enter"&&m.click()}),s.addEventListener("click",p=>{p.target===s&&s.remove()}),r.focus(),r.select()}window.deleteCatch=async function(t){if(k!==2){d("管理者権限が必要です",!0);return}if(!confirm(`この記録を削除しますか？
削除すると順位表も更新されます。`))return;const{error:e}=await b.from("catches").delete().eq("id",t);if(e){console.error("❌ 削除エラー:",e),d("❌ 削除に失敗しました",!0);return}d("✅ 削除しました"),await M(),await L()};async function L(){console.log("🏆 ランキング計算開始"),console.log("📋 現在のCONFIG:",u),console.log("📊 リミット匹数:",u.limit_count),console.log("🎯 大会ルール:",u.rule_type);const{data:t,error:e}=await b.from("catches").select("*").eq("tournament_id",h);if(e){console.error("❌ ランキング読み込みエラー:",e);return}const o=t||[];if(console.log("📊 釣果データ:",o.length,"件"),o.length===0){document.getElementById("ranking-list").innerHTML='<div class="empty-state">まだ釣果がありません</div>',document.getElementById("biggest-fish-list").innerHTML='<div class="empty-state">まだ釣果がありません</div>',document.getElementById("smallest-fish-list").innerHTML='<div class="empty-state">まだ釣果がありません</div>';return}const i={};w.forEach(c=>{i[c.zekken]=c});const n={};o.forEach(c=>{n[c.zekken]||(n[c.zekken]={zekken:c.zekken,lengths:[],weights:[],min_len:c.length,max_len:c.length,min_weight:c.weight||0,max_weight:c.weight||0}),n[c.zekken].lengths.push(c.length),n[c.zekken].weights.push(c.weight||0),n[c.zekken].min_len=Math.min(n[c.zekken].min_len,c.length),n[c.zekken].max_len=Math.max(n[c.zekken].max_len,c.length),n[c.zekken].min_weight=Math.min(n[c.zekken].min_weight,c.weight||0),n[c.zekken].max_weight=Math.max(n[c.zekken].max_weight,c.weight||0)});const l=Object.values(n).map(c=>{const f=[...c.lengths].sort((z,I)=>I-z),y=[...c.weights].sort((z,I)=>I-z),v=u.limit_count||999;console.log(`📊 選手${c.zekken}番の計算:`,{全釣果数:c.lengths.length,リミット匹数:v,全長寸:f,リミット長寸:f.slice(0,v)});const x=y.slice(0,v).reduce((z,I)=>z+I,0),E=f.slice(0,v).reduce((z,I)=>z+I,0);return{zekken:c.zekken,count:c.lengths.length,max_len:c.max_len,min_len:c.min_len,max_weight:c.max_weight,min_weight:c.min_weight,one_max_len:c.max_len,one_max_weight:c.max_weight,total_weight:c.weights.reduce((z,I)=>z+I,0),total_count:c.lengths.length,limit_weight:x,limit_total_len:E}}),s=u.rule_type||"max_len",r=u.sort1||null,a=u.sort2||null,g=u.sort3||null;l.sort((c,f)=>c[s]!==f[s]?f[s]-c[s]:r&&c[r]!==f[r]?f[r]-c[r]:a&&c[a]!==f[a]?f[a]-c[a]:g&&c[g]!==f[g]?f[g]-c[g]:0),A=l,console.log("✅ ランキング計算完了:",l.length,"人");const m=document.getElementById("show-biggest-fish")?.checked??!0;m?(document.querySelector(".prize-grid")?.style.setProperty("display","grid"),pe(l,i)):document.getElementById("biggest-fish-list").closest(".card").style.display="none";const p=document.getElementById("show-smallest-fish")?.checked??!0;p?(document.querySelector(".prize-grid")?.style.setProperty("display","grid"),ye(l,i)):document.getElementById("smallest-fish-list").closest(".card").style.display="none",!m&&!p&&document.querySelector(".prize-grid")?.style.setProperty("display","none"),K(l,i)}function pe(t,e){const o=document.getElementById("biggest-fish-list").closest(".card");o.style.display="block";const i=[...t].sort((r,a)=>a.max_len===r.max_len?a.max_weight-r.max_weight:a.max_len-r.max_len),n=new Set,l=[];for(const r of i)if(!n.has(r.zekken)&&(l.push(r),n.add(r.zekken),l.length===3))break;const s=document.getElementById("biggest-fish-list");s.innerHTML=l.map((r,a)=>{const g=e[r.zekken]||{},m=g.name||"未登録",p=g.club||"";return`
            <div class="ranking-item ${a===0?"top3":""}" style="padding: 8px; margin-bottom: 8px;">
                <div class="ranking-header">
                    ...
                    <div>
                        <div style="font-size: 14px; font-weight: bold;">${r.zekken}番: ${m}</div>
    ...
    `;document.body.insertAdjacentHTML("beforeend",o);const i=document.getElementById("edit-player-dialog"),n=document.getElementById("edit-name-input"),l=document.getElementById("edit-reading-input"),s=document.getElementById("edit-club-input"),r=document.getElementById("edit-cancel-btn"),a=document.getElementById("edit-ok-btn");r.onclick=()=>{i.remove(),e(null)},a.onclick=()=>{const g=n.value.trim(),m=l.value.trim(),p=s.value.trim();if(!g){d("名前は必須です",!0);return}i.remove(),e({name:g,reading:m,club:p})},n.addEventListener("keypress",g=>{g.key==="Enter"&&a.click()}),l.addEventListener("keypress",g=>{g.key==="Enter"&&a.click()}),s.addEventListener("keypress",g=>{g.key==="Enter"&&a.click()}),i.addEventListener("click",g=>{g.target===i&&(i.remove(),e(null))}),n.focus(),n.select()}window.addPlayer=async function(){if(k!==2){d("管理者権限が必要です",!0);return}const t=parseInt(document.getElementById("new-zekken").value),e=document.getElementById("new-name").value.trim(),o=document.getElementById("new-club").value.trim(),i=document.getElementById("new-reading").value.trim();if(!t||!e){d("ゼッケン番号と名前は必須です",!0);return}if(w.some(s=>s.zekken===t)){d(`${t}番は既に登録されています`,!0);return}const{error:l}=await b.from("players").insert({tournament_id:h,zekken:t,name:e,club:o||"",reading:i||""});if(l){console.error("選手追加エラー:",l),d("追加に失敗しました（重複の可能性）",!0);return}d("✅ 選手を追加しました"),document.getElementById("new-zekken").value="",document.getElementById("new-name").value="",document.getElementById("new-club").value="",document.getElementById("new-reading").value="",document.getElementById("zekken-warning").style.display="none",document.getElementById("add-player-btn").disabled=!1,await C(),await R()};let D=[];window.handleCSVFile=function(t){const e=t.target.files[0];if(!e)return;console.log("📂 CSVファイル選択:",e.name);const o=new FileReader;o.onload=function(i){const n=i.target.result;he(n)},o.readAsText(e,"UTF-8")};function he(t){try{console.log("📊 CSVパース開始");const e=t.split(/\r?\n/).filter(a=>a.trim());if(e.length<2){d("❌ CSVファイルが空です",!0);return}const i=e[0].split(",").map(a=>a.trim());console.log("📋 ヘッダー:",i);const l=["ゼッケン番号","名前"].filter(a=>!i.includes(a));if(l.length>0){d(`❌ 必須列が不足: ${l.join(", ")}`,!0);return}const s=[],r=[];for(let a=1;a<e.length;a++){const m=e[a].split(",").map(x=>x.trim());if(m.length!==i.length){r.push(`${a+1}行目: 列数が一致しません`);continue}const p={};i.forEach((x,E)=>{p[x]=m[E]});const c=parseInt(p.ゼッケン番号),f=p.名前;if(!c||isNaN(c)||c<=0){r.push(`${a+1}行目: ゼッケン番号が不正です (${p.ゼッケン番号})`);continue}if(!f||f.trim()===""){r.push(`${a+1}行目: 名前が空です`);continue}if(s.some(x=>x.zekken===c)){r.push(`${a+1}行目: ゼッケン番号 ${c} が重複しています`);continue}const v=w.find(x=>x.zekken===c);if(v){r.push(`${a+1}行目: ゼッケン番号 ${c} は既に登録されています (${v.name})`);continue}s.push({zekken:c,name:f,reading:p.読み仮名||"",club:p.所属||""})}if(console.log("✅ パース完了:",s.length,"件"),console.log("❌ エラー:",r.length,"件"),r.length>0){console.error("エラー詳細:",r),d(`⚠️ ${r.length}件のエラーがあります`,!0);const a=r.slice(0,5).join(`
`);alert(`CSVインポートエラー:

${a}${r.length>5?`

...他${r.length-5}件`:""}`)}if(s.length===0){d("❌ インポート可能なデータがありません",!0);return}D=s,be(s,r)}catch(e){console.error("❌ CSVパースエラー:",e),d("❌ CSVファイルの読み込みに失敗しました",!0)}}function be(t,e){const o=document.getElementById("csv-preview"),i=document.getElementById("csv-preview-content");let n=`
        <div style="margin-bottom: 15px;">
    ...
    `,i.innerHTML=n,o.style.display="block",console.log("👁️ プレビュー表示")}window.importCSV=async function(){if(D.length===0){d("❌ インポートするデータがありません",!0);return}if(k!==2){d("管理者権限が必要です",!0);return}console.log("🚀 CSVインポート開始:",D.length,"件");try{const t=D.map(i=>({tournament_id:h,zekken:i.zekken,name:i.name,reading:i.reading,club:i.club})),{data:e,error:o}=await b.from("players").insert(t).select();if(o){console.error("❌ インポートエラー:",o),d(`❌ インポートに失敗しました: ${o.message}`,!0);return}console.log("✅ インポート成功:",e.length,"件"),d(`✅ ${e.length}件の選手を登録しました！`),D=[],document.getElementById("csv-preview").style.display="none",document.getElementById("csv-file-input").value="",await C(),await R()}catch(t){console.error("❌ インポート例外:",t),d("❌ インポートに失敗しました",!0)}};window.cancelCSVImport=function(){D=[],document.getElementById("csv-preview").style.display="none",document.getElementById("csv-file-input").value="",d("インポートをキャンセルしました")};window.deletePlayer=async function(t){if(!confirm(`${t}番を削除しますか？`))return;const{error:e}=await b.from("players").delete().eq("tournament_id",h).eq("zekken",t);if(e){console.error("選手削除エラー:",e),d("❌ 削除に失敗しました",!0);return}d("✅ 削除しました"),await C(),await R()};const j={limit_total_len:"リミット合計長寸",limit_weight:"リミット合計重量",total_count:"枚数",total_weight:"総重量",one_max_len:"1匹最大長寸",one_max_weight:"1匹最大重量"};window.checkZekkenDuplicate=function(t){const e=document.getElementById("zekken-warning"),o=document.getElementById("add-player-btn");if(!t){e.style.display="none",o.disabled=!1;return}const i=parseInt(t);w.some(l=>l.zekken===i)?(e.textContent=`⚠️ ${i}番は既に登録されています`,e.style.color="#ff6b6b",e.style.fontWeight="bold",e.style.display="block",o.disabled=!0):(e.textContent=`✅ ${i}番は利用可能です`,e.style.color="#4CAF50",e.style.fontWeight="normal",e.style.display="block",o.disabled=!1)};window.updateSortOptions=function(){const t=document.getElementById("rule-type").value,e=document.getElementById("sort1").value,o=document.getElementById("sort2").value,i=[t];e&&i.push(e),o&&i.push(o),U("sort1",i,[t]),U("sort2",i,[t,e]),U("sort3",i,[t,e,o])};function U(t,e,o){const i=document.getElementById(t),n=i.value;i.innerHTML='<option value="">選択しない</option>';const l={one_max_len:"1匹最大長寸",one_max_weight:"1匹最大重量",limit_total_len:"リミット合計長寸",limit_weight:"リミット合計重量",total_count:"枚数",total_weight:"総重量"};for(const[s,r]of Object.entries(l))if(!o.includes(s)||s===n){const a=document.createElement("option");a.value=s,a.textContent=r,s===n&&(a.selected=!0),i.appendChild(a)}}async function xe(){if(console.log("⚙️ 大会設定読み込み開始"),!u||!u.id){console.error("❌ CONFIG が存在しません");return}document.getElementById("rule-type").value=u.rule_type||"limit_total_len",we(u.limit_count||0);const t=localStorage.getItem(`${h}_show_biggest_fish`),e=localStorage.getItem(`${h}_show_smallest_fish`);u.show_biggest_fish=t===null?!0:t==="true",u.show_smallest_fish=e===null?!0:e==="true",document.getElementById("show-biggest-fish").checked=u.show_biggest_fish,document.getElementById("show-smallest-fish").checked=u.show_smallest_fish,console.log("🏆 特別賞設定:",{show_biggest_fish:u.show_biggest_fish,show_smallest_fish:u.show_smallest_fish}),updateSortOptions(),document.getElementById("sort1").value=u.sort1||"",document.getElementById("sort2").value=u.sort2||"",document.getElementById("sort3").value=u.sort3||"",updateSortOptions(),console.log("✅ 大会設定読み込み完了:",u)}function we(t){const e=document.getElementById("limit-count-picker"),o=document.getElementById("limit-count"),i=e.querySelectorAll(".limit-option");o.value=t;const n=Array.from(i).find(r=>parseInt(r.dataset.value)===t);n&&(n.scrollIntoView({block:"center",behavior:"auto"}),s());let l;e.addEventListener("scroll",function(){clearTimeout(l),l=setTimeout(()=>{s()},100)}),i.forEach(r=>{r.addEventListener("click",function(){this.scrollIntoView({block:"center",behavior:"smooth"}),setTimeout(()=>s(),300)})});function s(){const r=e.getBoundingClientRect(),a=r.top+r.height/2;let g=null,m=1/0;i.forEach(p=>{const c=p.getBoundingClientRect(),f=c.top+c.height/2,y=Math.abs(a-f);y<m&&(m=y,g=p)}),g&&(i.forEach(p=>p.classList.remove("selected")),g.classList.add("selected"),o.value=g.dataset.value,console.log("📊 リミット匹数変更:",o.value))}}window.updateTournamentSettings=async function(){if(k!==2){d("管理者権限が必要です",!0);return}const t=document.getElementById("rule-type").value,e=parseInt(document.getElementById("limit-count").value)||0,o=document.getElementById("sort1").value,i=document.getElementById("sort2").value,n=document.getElementById("sort3").value,l=document.getElementById("show-biggest-fish").checked,s=document.getElementById("show-smallest-fish").checked;localStorage.setItem(`${h}_show_biggest_fish`,l),localStorage.setItem(`${h}_show_smallest_fish`,s);const r=[o,i,n].filter(y=>y!==""),a=new Set(r);if(r.length!==a.size){d("判定順位で同じ項目が選択されています",!0);return}console.log("💾 設定保存:",{ruleType:t,limitCount:e,sort1:o,sort2:i,sort3:n,showBiggestFish:l,showSmallestFish:s}),console.log("💾 更新条件:",{id:h}),console.log("💾 更新前のCONFIG.limit_count:",u.limit_count);const{data:g,error:m}=await b.from("tournaments").update({rule_type:t,limit_count:e,sort1:o||null,sort2:i||null,sort3:n||null}).eq("id",h).select();if(console.log("💾 UPDATE結果 - data:",g),console.log("💾 UPDATE結果 - error:",m),m){console.error("❌ 設定保存エラー:",m),console.error("❌ エラー詳細:",JSON.stringify(m,null,2)),console.error("❌ エラーコード:",m.code),console.error("❌ エラーメッセージ:",m.message),alert(`❌ 設定保存エラー: ${m.message}
コード: ${m.code}

⚠️ Supabase RLS UPDATE権限が設定されていない可能性があります。
CRITICAL_FIX.sqlを実行してください。`),d(`❌ 設定の保存に失敗しました: ${m.message||m.code||"不明なエラー"}`,!0);return}if(!g||g.length===0){console.error("❌ 更新対象が見つかりませんでした"),d("❌ 更新対象が見つかりませんでした",!0);return}console.log("✅ 更新後のデータ:",g);const{data:p,error:c}=await b.from("tournaments").select("*").eq("id",h).single();if(c||!p){console.error("❌ 設定再取得エラー:",c),d("❌ 設定の再取得に失敗しました",!0);return}u=p,u.show_biggest_fish=l,u.show_smallest_fish=s,console.log("✅ 再取得後のCONFIG:",u),d("✅ 設定を保存しました");const f=u.limit_count>0?`リミット${u.limit_count}匹`:"総力戦";document.getElementById("tournament-info").textContent=f,await L(),console.log("✅ 設定保存完了")};function d(t,e=!1){const o=document.getElementById("toast");o.textContent=t,o.className="toast"+(e?" error":""),o.style.display="block",setTimeout(()=>{o.style.display="none"},3e3)}let P=null;function ke(t,e){P=e,document.getElementById("confirm-message").textContent=t;const o=document.getElementById("confirm-dialog");o.style.display="flex"}window.confirmAction=function(){const t=document.getElementById("confirm-dialog");t.style.display="none",P&&(P(),P=null)};window.cancelConfirm=function(){const t=document.getElementById("confirm-dialog");t.style.display="none",P=null};console.log("✅ システム準備完了");function ve(){const t=document.getElementById("qrcode");t.innerHTML="";const e=window.location.origin+window.location.pathname+"?id="+h;document.getElementById("tournament-url").textContent=e,new QRCode(t,{text:e,width:200,height:200,colorDark:"#000000",colorLight:"#ffffff",correctLevel:QRCode.CorrectLevel.H}),console.log("✅ QRコード生成完了")}window.copyTournamentURL=function(){const t=document.getElementById("tournament-url").textContent;navigator.clipboard.writeText(t).then(()=>{d("✅ URLをコピーしました")}).catch(e=>{console.error("コピーエラー:",e),d("❌ コピーに失敗しました",!0)})};window.toggleTournamentStatus=async function(){if(k!==2){d("管理者権限が必要です",!0);return}const e=!(u.is_ended||!1),o=e?"終了":"再開";if(!confirm(`大会を${o}しますか？
${e?"終了すると釣果の入力ができなくなります。":"再開すると釣果の入力が可能になります。"}`))return;const{error:i}=await b.from("tournaments").update({is_ended:e}).eq("id",h);if(i){console.error("❌ 更新エラー:",i),d(`❌ ${o}に失敗しました`,!0);return}u.is_ended=e,W(),d(`✅ 大会を${o}しました`),X()};function W(){const t=u.is_ended||!1,e=document.getElementById("tournament-status-display"),o=document.getElementById("toggle-tournament-btn");t?(e.innerHTML="🔴 終了",e.style.background="rgba(255, 107, 107, 0.2)",e.style.borderColor="#ff6b6b",e.style.color="#ff6b6b",o.innerHTML="▶️ 大会を再開",o.style.background="linear-gradient(135deg, #51cf66 0%, #37b24d 100%)"):(e.innerHTML="🟢 進行中",e.style.background="rgba(81, 207, 102, 0.2)",e.style.borderColor="#51cf66",e.style.color="#51cf66",o.innerHTML="⏸️ 大会を終了",o.style.background="linear-gradient(135deg, #ffd93d 0%, #ff6b6b 100%)")}function X(){const t=u.is_ended||!1,e=document.getElementById("input-form");t&&k!==2&&(e.style.display="none",d("⚠️ 大会は終了しました",!0))}window.deleteTournament=async function(){if(k!==2){d("管理者権限が必要です",!0);return}const t=prompt(`大会を完全に削除します。
この操作は取り消せません。

削除する場合は、大会ID「`+h+"」を入力してください:");if(t!==h){t!==null&&d("❌ 大会IDが一致しません",!0);return}try{const{error:e}=await b.from("catches").delete().eq("tournament_id",h);if(e)throw e;const{error:o}=await b.from("players").delete().eq("tournament_id",h);if(o)throw o;const{error:i}=await b.from("tournaments").delete().eq("id",h);if(i)throw i;d("✅ 大会を削除しました"),setTimeout(()=>{window.location.href="/"},1500)}catch(e){console.error("❌ 削除エラー:",e),d("❌ 削除に失敗しました",!0)}};window.exportResults=async function(){if(k!==2){d("管理者権限が必要です",!0);return}try{const t=A||[],e=w||[];if(t.length===0){d("❌ エクスポートするデータがありません",!0);return}const{data:o,error:i}=await b.from("catches").select("*").eq("tournament_id",h).order("created_at",{ascending:!1});i&&console.error("釣果取得エラー:",i);const n=await ee(3),l=await te(3);let s="";s+=`【大会情報】
`,s+=`大会名,"${u.name||"釣り大会"}"
...
`}),s+=`
`,s+=`【特別賞】
`,console.log("🏆 特別賞チェック - biggestCatches:",n),console.log("🏆 特別賞チェック - smallestCatches:",l),n.length>0?(s+=`大物賞（長寸上位）
...
`}),s+=`
`,console.log(`✅ 大物賞を${n.length}件追加しました`)):console.log("⚠️ 大物賞データなし"),l.length>0?(s+=`最小寸賞（長寸下位）
`,s+=`順位,ゼッケン番号,名前,所属,長寸(cm),重量(g)
`,l.forEach((y,v)=>{const x=e.find(E=>E.zekken===y.zekken)||{};s+=`${v+1}位,${y.zekken}番,"${x.name||"未登録"}","${x.club||""}",${y.length},${y.weight||0}
`}),s+=`
...
`}));const a=u.name||"tournament",g=new Date().toISOString().split("T")[0],m=`${a}_完全版_${g}.csv`,p="\uFEFF",c=new Blob([p+s],{type:"text/csv;charset=utf-8;"}),f=document.createElement("a");f.href=URL.createObjectURL(c),f.download=m,f.click(),d("✅ CSVファイルをダウンロードしました")}catch(t){console.error("❌ エクスポートエラー:",t),d("❌ エクスポートに失敗しました",!0)}};document.addEventListener("DOMContentLoaded",function(){["zekken-number-input","length-input","weight-input"].forEach(e=>{const o=document.getElementById(e);o&&o.addEventListener("input",function(i){const n=i.target.value,l=Q(n);n!==l&&(i.target.value=l)})})});window.exportPDF=async function(){try{if(d("📄 PDF生成中..."),typeof window.jspdf>"u"||typeof html2canvas>"u"){d("❌ PDFライブラリが読み込まれていません",!0);return}const{jsPDF:t}=window.jspdf,e=A||[],o=w||[];if(e.length===0){d("❌ まだ釣果データがありません",!0);return}const n={limit_total_len:"リミット合計長寸",limit_weight:"リミット合計重量",total_count:"総枚数",total_weight:"総重量"}[u.rule_type]||"リミット合計長寸",l=u.limit_count>0?`(リミット${u.limit_count}匹)`:"(無制限)",s=document.createElement("div");s.style.cssText=`
            position: absolute;
            left: -9999px;
            width: 800px;
            background: white;
